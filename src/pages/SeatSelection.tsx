...
function generateMockSeats(): SeatData[] {
  ...
  const sections: Record<string, Section> = {
    VVIP: { section_id: 1, section_name: "VVIP", price: 3200.0, color: "#E10600" },
    VIP: { section_id: 2, section_name: "VIP", price: 2400.0, color: "#B00500" },
  };
...
export default function SeatSelection() {
  ...
  // Calculate totals
  const ticketTotal = useMemo(() => selectedSeats.reduce((sum, seat) => sum + seat.section.price, 0), [selectedSeats]);
        ...
        <Tooltip delayDuration={100}>
          <TooltipTrigger asChild>
            ...
            >
              ...
              <span className="text-[8px] md:text-[10px] opacity-80">{seat.section.price.toLocaleString()}</span>
            </button>
          </TooltipTrigger>
          {seat.status !== "occupied" && (
            <TooltipContent className="bg-[#1A1D25] border-white/20 text-white" side="top">
  ...
  const renderSeatsWithAisle = (rowSeats: SeatData[]) => {
    ...
    return (
      <div className="flex items-center justify-center gap-1 md:gap-1.5">
        {/* Left section */}
        <div className="flex gap-1 md:gap-1.5">{leftSeats.map((seat) => renderSeat(seat))}</div>

        {/* Center aisle */}
        <div className="w-6 md:w-10" />

        {/* Right section */}
        <div className="flex gap-1 md:gap-1.5">{rightSeats.map((seat) => renderSeat(seat))}</div>
      </div>
    );
  };
  ...
  const handleProceed = () => {
    ...
    alert(
      `Proceeding to payment for ${selectedSeats.length} seats: ${selectedSeats.map((s) => `${s.row_label}${s.seat_number}`).join(", ")}\nTotal: LKR ${ticketTotal.toLocaleString()}`,
    );
  };
                ...
                {showtimeData.map((showtime, idx) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedTime === showtime.time ? "border-primary" : "border-transparent hover:border-white/30",
                    )}
                  >
              ...
              {rowOrder.map((rowLabel) => {
                ...
                return (
                  <div key={rowLabel} className="flex items-center gap-3 md:gap-4 min-w-max justify-center">
                    {/* Left Row Label */}
                    <span className="w-8 text-center text-sm font-bold text-primary">{rowLabel}</span>
        ...
        <div className="p-4 flex-1 overflow-auto">
          ...
          <div className="flex gap-3 mb-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-white/80 mt-2">
                {dates[selectedDateIndex].dayName.slice(0, 3)}, {dates[selectedDateIndex].day}{" "}
                {dates[selectedDateIndex].month}, {selectedTime}
              </p>
          ...
          {Object.entries(selectedSeatsBySection).map(([sectionName, { section, seats: sectionSeats }]) => (
            <div key={sectionName} className="mb-6">
              ...
              <p className="text-sm font-medium mb-2" style={{ color: section.color }}>
              ...
              {Object.entries(selectedSeatsBySection).map(([sectionName, { section, seats: sectionSeats }]) => (
                <div key={sectionName} className="flex justify-between text-sm mb-1">
                  <span className="text-white/80">
                    {sectionSeats.length} x {section.price.toLocaleString()}
                  </span>
